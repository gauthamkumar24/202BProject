...
#if ( ( configGENERATE_RUN_TIME_STATS == 1 ) && ( configUSE_STATS_FORMATTING_FUNCTIONS > 0 ) )
void vCpuStats(unsigned long* cpuidletime)
{   //vTaskSuspendAll();   
      //vTaskSwitchContext();
  static unsigned long PREVIOUS_CPU_IDLE_TICKS=0;
       TaskHandle_t xHandle;
       TaskStatus_t xTaskDetails;
      //skGetRunTimeStats(cBuffer);
      //vCpuIdleTime( cpuidletime, runtime );
       xHandle=xTaskGetIdleTaskHandle();
      vTaskGetInfo(xHandle,&xTaskDetails,pdTRUE,eInvalid);
      *cpuidletime=xTaskDetails.ulRunTimeCounter-PREVIOUS_CPU_IDLE_TICKS;
      PREVIOUS_CPU_IDLE_TICKS=xTaskDetails.ulRunTimeCounter;
...
/*
#if ( ( configGENERATE_RUN_TIME_STATS == 1 ) && ( configUSE_STATS_FORMATTING_FUNCTIONS > 0 ) )
void vCpuStats_Average(unsigned long* cpuidletime)
{ 
       TaskHandle_t xHandle;
       TaskStatus_t xTaskDetails;
      //skGetRunTimeStats(cBuffer);
...
/*-----------------------------------------------------------*/
	...
	void vCpuIdleTime( uint32_t cpuidletime,uint32_t runtime )
	{
	TaskStatus_t *pxTaskStatusArray;
	volatile UBaseType_t uxArraySize, x;
	uint32_t ulTotalTime, ulStatsAsPercentage;
		...
		if( pxTaskStatusArray != NULL )
		{
			...
			if( ulTotalTime > 0 )
			{
				...
				for( x = 0; x < uxArraySize; x++ )
				{
					...
					easily. */
					//pcWriteBuffer = prvWriteNameToBuffer( pcWriteBuffer, pxTaskStatusArray[ x ].pcTaskName );
					if(pxTaskStatusArray[ x ].pcTaskName== "IDLE" )
					{
								
						cpuidletime=  pxTaskStatusArray[ x ].ulRunTimeCounter;
